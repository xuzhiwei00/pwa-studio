...
const Tree = props => {
    const { categories, categoryId, onNavigate, query, setCategoryId, updateCategories } = props
    ...
    const branches = rootCategory ? Array.from(children || [], id => (
        <Branch
            key={id}
    ...
    const leaf = rootCategory && url_path ? <Leaf category={rootCategory} onNavigate={onNavigate} /> : null
